...
int main(int argv, char ** argc)
{
    ...
    while(1)
    {
        ...
        printf("[Thread 0]: receive pocket_size byte : %d\n", pocket_size);
        handle_pocket(buff, pocket_size, recv_from, dnsmap,my_socket, 0);
    }
    // 结束WSA
    WSACleanup();
    return 0;
...
void send_out(char * buff, int buff_size, struct sockaddr_in recv_from, SOCKET my_socket, int thread_id)
{
    ...
    out_addr.sin_family = AF_INET; // 地址族
    out_addr.sin_addr.S_un.S_addr = inet_addr( DNS_server.c_str() );  // default 10.3.9.4
    ...
    THREADDEBUG <<"Send successfully to " <<DNS_server<<endl;
    int tv = 500;
	if (setsockopt(out_socket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof(tv)) < 0)
    {
		perror("Error timer\n");
	}
    ...
    if(ret_size <= 0)
    {
        THREADDEBUG <<"RECVFROM FUNCTION TIME OUT!\n";
        return;
    }
    else
    {
...
//            send_out(buff, pocket_size, recv_from, my_socket, thread_id);
        ...
        if(QuireMap.count(domain_name))  // 存在本地文件中
        {
            ...
            if(!strcmp(iter->second.c_str(), "0.0.0.0"))
                flag = 1;
            else if(tmp[i + 2] == 28)  // IPv6 存入
            {
                THREADDEBUG << "save a pocket " <<pocket_size << endl;
                pool_mutex.lock();
                for(int i = 0; i < MAXSIZE; i++)
                {
